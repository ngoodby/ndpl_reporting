...
In `r this_year`, we confirmed the presence of `r total_species` bat species (see Table \@ref(tab:speciesthisyear)):
```{r}
for (i in seq(1:length(report_grts))){
  cat(total_species_by_grts_this_year[i,]$n, "species in GRTS", total_species_by_grts_this_year[i,]$grts_cell_id, "\n")
}
```

Over all years of monitoring, we confirmed `r total_species_all_years` bat species (see Table \@ref(tab:speciesallyears)):
`r species_2020` in 2020 and `r species_2021` in GRTS cell 2021.
```{r}
for (i in seq(1:nrow(total_species_by_year))){
  cat(total_species_by_year[i,]$n, "species in ", total_species_by_year[i,]$year, "\n")
}
```
