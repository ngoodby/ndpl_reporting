...
partner <- "USFS"
table_path <- "../species tables/California.png"
...
```{r}
#
spp_list <- googlesheets4::read_sheet('https://docs.google.com/spreadsheets/d/1UggTTGxYYmVM2FVpjHqn-q0AZLb-oqGKlJytg7r5JXg/edit?usp=sharing', sheet = "SB_Species_List") %>% mutate(keyword = strsplit(Species_List, ",")) %>% 
...
all_total_species_106<- dat_count %>% dplyr::filter(pres_abs == TRUE, grts_cell_id == 106) %>% group_by(grts_cell_id) %>% summarise(n = n_distinct(species)) %>% dplyr::pull(n)

all_total_species_12858<- dat_count %>% dplyr::filter(pres_abs == TRUE, grts_cell_id == 2858) %>% group_by(grts_cell_id) %>% summarise(n = n_distinct(species)) %>% dplyr::pull(n)

species_2020 <- dat_count %>% dplyr::filter(pres_abs == TRUE, year == 2020) %>% group_by(year) %>% summarise(n = n_distinct(species)) %>% dplyr::pull(n)

species_2021 <- dat_count %>% dplyr::filter(pres_abs == TRUE, year == 2021) %>% group_by(year) %>% summarise(n = n_distinct(species)) %>% dplyr::pull(n)

species_per_106 <- dat_count %>%
  dplyr::filter(year == this_year, pres_abs == TRUE, grts_cell_id == 106) %>% 
  group_by(grts_cell_id) %>% 
  summarise(n = n_distinct(species)) %>% dplyr::pull(n)

species_per_2858 <- dat_count %>%
  dplyr::filter(year == this_year, pres_abs == TRUE, grts_cell_id == 2858) %>% 
  group_by(grts_cell_id) %>% 
  summarise(n = n_distinct(species)) %>% dplyr::pull(n)

```

In `r this_year`, we confirmed the presence of `r total_species` bat species (see Table \@ref(tab:speciesthisyear)): `r species_per_106` in GRTS cell 106 and `r species_per_2858` in GRTS cell 2858.

Over all years of monitoring, we confirmed `r all_total_species` bat species (see Table \@ref(tab:speciesallyears)):`r species_2020` in 2020 and `r species_2021` in GRTS cell 2021.
