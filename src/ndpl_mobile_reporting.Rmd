...
mobile_data <- load_mobile_data(username, password, project_id, report_grts, mobile_exclude)
...
mobile_all_dat <- mobile_all_dat %>% 
  # left_join(species_reference_table, 
  #   by = c("species_code" = "species_code", 
  #  "species_code_6"="species_code_6"),
  # mutate("common_name" = common_name.y) %>% dplyr::select(-common_name.x, -common_name.y) %>% 
  left_join(detector_codes, by = c("detector_type_id"="id")) %>% 
  left_join(mic_codes, by = c("microphone_type_id"="id"))
...
for (i in mobile_grts){
  transect_info <- get_mobile_route(username, password, grts_id, project_id)
}
...
grts_mapping <- grts.grid %>% 
  st_as_sf() %>%
  dplyr::filter(GRTS_ID %in% mobile_grts) %>% 
  dplyr::filter(country == "US")

mobile_detection_coords <- transect_info[["transect"]][["coordinates"]] 
mobile_detections <- data.frame(matrix(unlist(mobile_detection_coords), nrow=length(mobile_detection_coords), byrow=TRUE))

mobile_detections %>% leaflet() %>% 
  addProviderTiles("Esri.WorldTopoMap") %>% 
  addPolylines(lat = ~X2, lng = ~X1, weight = 3, color = 'red') %>% 
