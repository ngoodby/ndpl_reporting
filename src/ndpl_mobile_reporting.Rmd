...
Species that may occur in surveyed area of the `r partner` and the specific and general categories in which they are included. Species of Greatest Conservation Need (SGCN), as designated by the State's Wildlife Action Plan, are bolded.
...
species_reference_table %>% dplyr::filter(species_code_6 %in% spp_list$name) %>% 
  mutate(freq_range = case_when(
    species_code %in% available_sgrams ~ sprintf("![%s](../sonograms/%s_sgram.png)", freq_range, species_code),
  ...
  mutate(species_scientific = case_when(
    ...
    !is.na(species_url) & (species_scientific %in% sgcn) ~ sprintf("[*%s* (SGCN)](%s)", species_scientific, species_url)
  )) %>% 
  dplyr::select(species_scientific, species_scientific, common_name, freq_range) %>%
  rename("Scientific Name" = species_scientific, 
         "Common Name" = common_name,
         "Characteristic Frequency Range (kHz)" = freq_range) %>%
  kable()
```
...
if(length(sppcon) > 0){
  cat(paste(sppcon, collapse= ', '), "were first confirmed this year")
} else {
...
## Mobile Acoustic Survey Metadata
