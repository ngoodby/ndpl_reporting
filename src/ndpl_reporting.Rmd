...
freqs <- species_reference_table %>% dplyr::filter(species_scientific %in% spp_list$species_scientific) %>%
  ...
  dplyr::select(species_scientific, species_code, species_url, common_name, freq_range)
```
...
for (i in seq(1, nrow(freqs))){
  ...
  cat(sprintf('<summary>\n%s - <a href="%s" target="_blank">%s kHz</a>\n</summary>\n', 
            freqs[i,]$species_scientific, freqs[i,]$species_code, freqs[i,]$species_url, freqs[i,]$freq_range))
