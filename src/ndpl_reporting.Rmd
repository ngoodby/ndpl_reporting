...
![](../logos/BCI_logo.png){height=100} ![](../logos/CMI_logo.png){height=100} 
...
```{r results = 'asis'}
if (report_type=="mobile"){
  cat("For mobile transects, every file not scrubbed out as noise was manually viewed in Sonobat.")
}
```
...
Species that may occur in surveyed area of the `r partner` and their characteristic frequncy range. Click on the arrows to view an example sonogrgam for each species. Click on the links to learn more about that species.
...
freqs <- species_reference_table %>% dplyr::filter(species_code_6 %in% spp_list$name) %>%
  ...
  mutate(species_scientific = case_when(
    !is.na(species_url) & (species_scientific %in% sgcn) ~ sprintf("*%s* (SGCN in %s)", species_scientific, kpro_region),
    T ~ paste0("*", species_scientific, "*")
  )) %>% 
  dplyr::select(species_scientific, species_url, common_name, freq_range, species_code)
```
...
![](../logos/NABat_logo.png){height="200"}
