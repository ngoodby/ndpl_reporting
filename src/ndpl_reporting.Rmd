...
report_grts_stationary= "10411"
report_grts_mobile= "2543"
project_id = 494
report_locations = c("")
forest_id <- "ShawneeNF"
partner <- "US Forest Service"
report_reference <- "Shawnee National Forest"
...
data <- load_nabat_data(username, password, project_id, report_grts_stationary, report_locations, exclude)
all_dat <- data[[1]]
dat_count <- data[[2]]
sa_survey_df <- data[[3]]
project_df <- data[[4]]
...
if (report_type == "mobile"){
  ...
  mobile_data <- load_mobile_data(username, password, project_id, report_grts_mobile)
...
lookup_t <- googlesheets4::read_sheet('https://docs.google.com/spreadsheets/d/1UggTTGxYYmVM2FVpjHqn-q0AZLb-oqGKlJytg7r5JXg/edit?usp=sharing', sheet = "2021")
