...
files <- list.files(path = here::here("site_photos"), 
                    ...
                    full.names = T,
                    recursive = T)
short_path <- list.files(path = here::here("site_photos"), 
                    pattern = "^.*.jpg$",
                    full.names = F,
                    recursive = T)

#create captions for photos
cells <- short_path %>% strsplit( "/" ) %>% sapply( "[", 1 )
sites <-  short_path %>% strsplit( "/" ) %>% sapply( "[", 2 )
captions <- str_c("NABat Cell ", cells, "; Survey Site: ", sites, sep = "")
```

<details>
  <summary>Click to View Site Photos</summary>
