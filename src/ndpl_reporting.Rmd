...
if (!dir.exists(here::here("geojson_files"))) {dir.create(here::here("geojson_files"))}
...
# known issue that include_graphics doesn't work in a for loop. Need to update this.

 for (i in seq(1, length(pics))){
  for (pic in seq(1, length(pics[[i]]))){
    photo <- pics[[i]][pic]
    image_write(photo, "my_photo.jpg")
    knitr::include_graphics("my_photo.jpg")
    mask <- photo_queries[i,] %>% dplyr::select(survey_event_id) %>% pull()
    print(sa_survey_df %>% dplyr::filter(survey_event_id == mask) %>% dplyr::pull(grts_cell_id, event))
  }
}
```
