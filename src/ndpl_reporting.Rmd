...
```{r}
# table <- read_csv(here::here("data","species_reference_table.csv"))
# table_test <- separate(table, freq_range, into=c("low_freq","high_freq"), sep = "-", remove=T)
# write_csv(table_test, here::here("data", ""))
```
...
detector <- all_dat %>% filter(year == this_year) %>% {unique(.$detector_type_id)} %>% as.list()
```

In `r this_year`, the `r partner` implemented acoustic bat monitoring protocols described in _A Plan for the North American Bat Monitoring Program (NABat)_. Stationary detectors were deployed within GRTS Cells `r report_grts` for four consecutive nights each. The results of the stationary detectors are presented here. `r detector` was used to record full-spectrum echolocation files. Figure \ref{fig:map} shows the location of each detector within the cell, and Figure 2 shows the habitat at each site.
...
1. Confirming species as assigned by the program; 

2. Assigning the file to a species couplet, or frequency category based on the frequency and call shape (Table 1); or 

3. Assigning the files as noise, i.e., not a recording of bat echolocation calls. We reviewed files in order of confidence in identification as expressed by the program. 

A confirmed identification of a species in 1 or more files on a given night at a given site resulted in a designation of species presence for that site-night. Identification of up to three files added an additional measure of confidence to species presence designations. When we could not assign any files recorded on a site-night to species, that species was considered “not detected.”
...
![Species that may occur in jurisdiction of the `r partner` and the specific and general categories in which they are included. ](`r table_path`)
...
filtered_all_dat <- all_dat %>% dplyr::filter(!species_code %in% exclude) %>%
...
species_table_this_year <- filtered_all_dat %>% 
  dplyr::filter(year == this_year & !is.na(species_code)) %>% 
...
species_table_this_year %>%
  flextable(col_keys = names(.)) %>% 
  merge_v(., j=c("location_name", "grts_cell_id")) %>% 
  theme_box() %>%
  flextable::align(., align = "center", part = "all") %>%
  set_table_properties(., width = 1, layout = "autofit")
```
