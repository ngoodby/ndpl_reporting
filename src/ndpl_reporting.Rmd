...
![](../logos/BCI_logo.png){height=80}   ![](../logos/CMI_logo.jpg){height=80} 
...
In `r this_year`, the `r partner` implemented acoustic bat monitoring protocols described in _A Plan for the North American Bat Monitoring Program (NABat)_. Stationary detectors were deployed within `r length(report_grts)` GRTS cell(s) for four consecutive nights each. Full spectrum echolocation files were recorded using the detectors in Table \@ref(tab:sitemeta). Figure \@ref(fig:map) shows the location of each detector within the cell, and Section  \@ref(site-photos) shows the habitat at each site.

```{r results = 'asis'}
if (report_type=="mobile"){
  cat(sprintf('Mobile acoustic surveys were conducted on %s nights in %s. Figure \\@ref(fig:mobilemap) shows all mobile transect routes conducted. Full spectrum echolocation files were recorded using the detectors in Table \\@ref(tab:sitemeta)', n_distinct(mobile_all_dat$recording_night), this_year))
}
```
...
For stationary data, we attempted to confirm the identification of a maximum of three files per species, per night, per site by visually inspecting the species ID proposed by the programs. Up to 10 files per species-site-night were reviewed in an attempt to confirm the species ID of 3 files.
