...
The resulting recording files were sent to the National Data Processing Lab (NDPL) for processing. Acoustic recordings were aligned with metadata, renamed, and attributed using a custom R and Python code developed by Conservation Metrics Inc (CMI) and the United States Geological Survey (USGS). Files were then scrubbed of noise using Kaleidoscope Pro 5.3.9 and were additionally run through the auto-ID program set to the `r kpro_region`. Files that were not scrubbed out as noise were then run through Sonobatch in Sonobat 4.4 set to the `r classifier` classifier region (Section  \@ref(potential-species)) to determine call parameters and the suggested species identification. Use of both programs is to provide additional automated identification used in manual vetting. 
