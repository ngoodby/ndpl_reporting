...
freqs <- species_reference_table %>% dplyr::filter(species_scientific %in% spp_list$name) %>%
...
for (i in seq(1, nrow(freqs))){
  cat("<details>\n")
  cat(sprintf('<summary>\n%s - <a href="%s" target="_blank">%s kHz</a>\n</summary>\n', 
            freqs[i,]$species_scientific, freqs[i,]$species_url, freqs[i,]$freq_range))
...
```{r species confirmed for the first time this year}
if(length(sppcon) == 1){
...
All acoustic results were uploaded to NABat project `r project_df$project_name[1]` and can be viewed `r proj_link`. *Note that you need to have a NABat account and be a member of the project to be able to view the results on NABat. If you would like to view these results on NABat and don't have an account, please contact us.*
